...
class Robot:
    ...
    def apply_action(self, action, use_angles=False):
        if not len(action) == 3:
    ...
    def apply_action_joint_angles(self, action):
        assert len(action) == 8
        joint_angles, gripper_width = action[:7], action[-1]
        ...
        self.control_gripper(gripper_width)


    def control_gripper_width(self, gripper_width):
