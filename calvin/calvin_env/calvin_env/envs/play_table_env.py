...
class PlayTableSimEnv(gym.Env):
    ...
    def step(self, action, use_angles=False):
        ...
        if self.use_vr:
            ...
            if delta_t >= (1.0 / self.control_freq):
                ...
                self.prev_time = time.time()
                self.robot.apply_action(action, use_angles)
            self.fps_controller.step()
        # for RL call step simulation repeat
        else:
            if use_angles:
                self.robot.apply_action_joint_angles(action)
            else:
...
def get_env(dataset_path, obs_space=None, show_gui=True, **kwargs):
    ...
    render_conf = OmegaConf.load(Path(dataset_path))
