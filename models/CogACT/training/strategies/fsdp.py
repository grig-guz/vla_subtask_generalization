...
from utils.calvin_utils import load_cogact_checkpoint, get_calvin_env
    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_save_optimizer_policy):
            ...
            if overwatch.is_rank_zero():
                ...
                checkpoint_path = checkpoint_dir / f"checkpoint.pt"
                ...
                print("THE PRETRAINED CHECKPOINT ", str(checkpoint_path))
                policy, _ = load_cogact_checkpoint(str(checkpoint_path), model_state_dict=model_state_dicts)
                new_rate = run_validation(policy=policy, eval_type=data_mix, model_name='cogact', global_step=global_step)

                if new_rate > self.best_score:
                    print(f"New best score: {new_rate}, previous: {self.best_score}, saving the checkpoint")
                    self.best_score = new_rate
                    torch.save({"model": model_state_dicts}, checkpoint_path)
                else:
                    print(f"Current rate: {new_rate}, previous: {self.best_score}, not saving the checkpoint")
