...
class AdaptiveEnsembler:
    ...
    def ensemble_action(self, cur_action):
        ...
        if cur_action.ndim == 1:
            # Single action, just stack all actions in history (including current one)
            curr_act_preds = np.stack(self.action_history)
        else:
            # future_action_window_size actions, select actions corresponding to the current timestep from previous predictions
