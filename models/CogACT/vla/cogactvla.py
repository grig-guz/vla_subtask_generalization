...
class CogACT(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if model_state_dict == None:
            model_state_dict = torch.load(pretrained_checkpoint, map_location=f"cuda:{device}")["model"]
        ...
        print("Put state dict into VLA!")
        # Freeze Weights
        if freeze_weights:
            vlm.requires_grad_(False)
            vlm.eval()
        print("Passed setting no grad!")
        # Initialize CogACT
        cogact = CogACT(vlm,
    ...
    def predict_action(
        ...
        **kwargs: str
        ...
        # For non-gripper actions, apply 1/2 * (act + 1) * (act_high - act_low) + act_low
        actions = np.where(
            mask,
