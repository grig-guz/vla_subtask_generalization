...
def merge_params(target_params: Params, pretrained_params: Params, random_init: bool=False) -> Params:
    ...
    if random_init:
        ...
        for k in flat_target_params:
            if k in flat_pretrained_params and flat_target_params[k].shape == flat_pretrained_params[k].shape and 'hf_model' in k:
                keys_to_update.append(k)
                print("Random init: will merge the key ", k)
            else:
                print("Random init: randomly initialize the key ", k)
    else:
        keys_to_update = [
            k
            for k in flat_target_params
