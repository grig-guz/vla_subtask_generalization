...
from models.octo.octo.data import obs_transforms, traj_transforms
from models.octo.octo.data.utils import goal_relabeling, task_augmentation
from models.octo.octo.data.utils.data_utils import (
    allocate_threads,
...
from models.octo.octo.utils.spec import ModuleSpec
import pickle
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def keep_episode(example, keep_ids):
        ...
        eid = tf.strings.regex_replace(ep_name, r'^.*_', '')
        eid_num = tf.strings.to_number(eid, out_type=tf.int32)
        return tf.reduce_any(tf.equal(keep_ids, eid_num))
    ...
    if isinstance(dataset_statistics, str):
        ...
        if do_subsampling:
            full_dataset = full_dataset.filter(lambda x: keep_episode(x, keep_ids))
    ...
    if do_subsampling:
        dataset = dataset.filter(lambda x: keep_episode(x, keep_ids))
