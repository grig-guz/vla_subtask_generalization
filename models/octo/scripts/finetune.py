...
from utils.shared_utils import run_validation
...
FLAGS = flags.FLAGS
flags.DEFINE_string("name", "experiment_2", "Experiment name.")
...
def main(_):
    ...
    initialize_compilation_cache(cache_dir=FLAGS.config.jax_compilation_path)
    devices = jax.devices()
    logging.info(
        f"""
    ...
    # Our model will be replicated across devices (we arse only doing data parallelism, not model parallelism)
    ...
    wandb_id = "{name}_{time}_{dataset}_{size}_{random_init}_{seed}".format(
        name=name,
        time=datetime.datetime.now().strftime("%Y%m%d_%H%M%S"),
        dataset=FLAGS.config.dataset_kwargs.name,
        size="small" if "small" in FLAGS.config.pretrained_path else "base", 
        random_init=FLAGS.config.random_init,
    ...
    if 'wrist' in dict(FLAGS.config.dataset_kwargs.image_obs_keys):
        ...
        save_dir = tf.io.gfile.join(
            ...
            "random_" + str(FLAGS.config.random_init),
            wandb_id,
        )
    ...
    dataset = make_single_dataset(
        ...
        train=True,
        ann_dict=ann_dict,
        do_subsampling=FLAGS.config.do_subsampling,
        save_dir=save_dir
    )
    
    # 1) Repeat trajectories
    # 2) Turn trajectories dataset into dataset of timesteps.
    # 3) Shuffle timesteps
    # 4) Create batches 
    train_data_iter = (
        dataset.repeat()
        .unbatch()
        .shuffle(FLAGS.config.shuffle_buffer_size, reshuffle_each_iteration=True)
    ...
    #
    #########
    if FLAGS.config.traj_transform_kwargs['action_horizon'] != 4:
    ...
    merged_params = merge_params(model.params, pretrained_model.params, random_init=FLAGS.config.random_init)
    ...
    for i in tqdm.tqdm(
        ...
        if (i + 1) % FLAGS.config.save_interval == 0 and save_dir is not None:
            ...
            new_avg_rate = run_validation(policy=train_state.model, 
                eval_type=FLAGS.config.dataset_kwargs.name, 
                model_name='octo', 
                global_step=i
            )
            
            if new_avg_rate > best_rate:
                print(f"Achieved new highest success rate at timestep {i}: {new_avg_rate}, before: {best_rate}")
                best_rate = new_avg_rate
                print("Saving checkpoint...")
                save_callback(train_state, i + 1)
            else:
                print(f"Checkpoint at timestep {i} did not achieve higher success rate: {new_avg_rate}, best: {best_rate}")
