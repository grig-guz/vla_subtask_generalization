...
class LeRobotCalvinDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[libero_policy.CalvinInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[libero_policy.CalvinOutputs()],
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        # Change the name to reflect your model and dataset.
        name="libero_high_level",
        # Here you define the model config -- In this example we use pi0 as the model
        # architecture and perform *full* finetuning. in the examples below we show how to modify
        # this to perform *low-memory* (LORA) finetuning and use pi0-FAST as an alternative architecture.
        model=pi0_fast.Pi0FASTConfig(action_dim=7, action_horizon=10, max_token_len=250),
        # Here you define the dataset you are training on. In this example we use the Libero
        # dataset. For your own dataset, you can change the repo_id to point to your dataset.
        # Also modify the DataConfig to use the new config you made for your dataset above.
        data=LeRobotLiberoDataConfig(
            repo_id="libero_high_level",
            base_config=DataConfig(
                local_files_only=False,  # Set to True for local-only datasets.
                # This flag determines whether we load the prompt (i.e. the task instruction) from the
                # ``task`` field in the LeRobot dataset. If set to True, the prompt will show up in
    ...
    TrainConfig(
        ...
        fsdp_devices=4,
        save_interval=5000,
        checkpoint_base_dir = "/checkpoint/" + os.getenv("USER") + "/" + os.getenv("SLURM_JOB_ID")
    ),
    TrainConfig(
        # Change the name to reflect your model and dataset.
        name="calvin_conj",
