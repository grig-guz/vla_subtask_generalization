...
from utils.calvin_utils import add_text
...
class RolloutVideo:
    ...
    def draw_relabel_outcome(self, successful, video_idx, time_idx):
        """
        Draw red or green border around video depening on successful execution
        and repeat last frames.
        Args:
        ...
        border = 3
        frames = 5
        self.videos[video_idx][:, time_idx:time_idx+1, c, :, :border] = 1
        self.videos[video_idx][:, time_idx:time_idx+1, not_c, :, :border] = 0
        self.videos[video_idx][:, time_idx:time_idx+1, c, :, -border:] = 1
        self.videos[video_idx][:, time_idx:time_idx+1, not_c, :, -border:] = 0
        self.videos[video_idx][:, time_idx:time_idx+1, c, :border, :] = 1
        self.videos[video_idx][:, time_idx:time_idx+1, not_c, :border, :] = 0
        self.videos[video_idx][:, time_idx:time_idx+1, c, -border:, :] = 1
        self.videos[video_idx][:, time_idx:time_idx+1, not_c, -border:, :] = 0
        repeat_frames = torch.repeat_interleave(self.videos[video_idx][:, time_idx:time_idx+1], repeats=frames, dim=1)
        self.videos[video_idx] = torch.cat([self.videos[video_idx][:, :time_idx], repeat_frames, self.videos[video_idx][:, time_idx+1:]], dim=1)
        self.step_counter += frames
    ...
    def update(self, rgb_obs: torch.Tensor, step=None) -> None:
        """
        Add new frame to video.
        Args:
            rgb_obs: static camera RGB images
        """
        img = rgb_obs.detach().cpu() / 255.0
        if step != None:
            img_text = np.zeros((224, 448, 3), dtype=np.uint8) + 127
            add_text(img_text, "step: " + str(step), loc='top')
            img_text = ((img_text.transpose(2, 0, 1).astype(float) / 255.0) * 2) - 1
            img = img + torch.from_numpy(img_text)
    ...
    def update_qval(self, rgb_obs: torch.Tensor, qval) -> None:
        """
        Add new frame to video.
        Args:
            rgb_obs: static camera RGB images
        ...
        if len(self.videos) > 0:
            ...
            img_text = np.zeros((224, 224, 3), dtype=np.uint8) + 127
            img_text = ((img_text.transpose(2, 0, 1).astype(float) / 255.0) * 2) - 1
            img = _unnormalize(img) + torch.from_numpy(img_text)
            self.videos[-1] = torch.cat([self.videos[-1], img], dim=1)  # shape 1, t, c, h, w
        else:
            img = rgb_obs.detach().cpu()
    ...
    def add_language_instruction(self, instruction, video_idx=-1, loc='bot'):
        img_text = np.zeros(self.videos[video_idx].shape[2:][::-1], dtype=np.uint8) + 127
        img_text = img_text[:224, :, :]
        add_text(img_text, instruction, loc=loc)
        img_text = ((img_text.transpose(2, 0, 1).astype(float) / 255.0) * 2) - 1
        self.videos[video_idx][:, self.sub_task_beginning :, :, :, :224] += torch.from_numpy(img_text)
        self.videos[video_idx] = torch.clip(self.videos[video_idx], -1, 1)


    def add_init_state(self, rgb_init, final_state=None, video_idx=-1):
        self.videos[video_idx][:, self.sub_task_beginning:, :, :, 224:448] += _unnormalize(rgb_init.detach().cpu())  # shape 1, t, c, h, w
        if final_state is not None:
            self.videos[video_idx][:, self.sub_task_beginning:, :, :, 448:] += _unnormalize(final_state.detach().cpu())
    ...
    def write_to_tmp(self):
        ...
        """
        if not self.log_to_file and isinstance(self.logger, WandbLogger):
    ...
    def _log_videos_to_file(self, global_step, save_as_video=False, ):
        ...
        for video, tag in zip(self.videos, self.tags):
            ...
            print("Video save location: ", filename)
            if save_as_video:
